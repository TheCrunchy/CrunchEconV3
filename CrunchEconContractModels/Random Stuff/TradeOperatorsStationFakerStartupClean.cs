...
{
    public static class TradeOperatorsStationFakerStartupClean
    {
            ...
            if (entity is MyCubeGrid grid)
            {
                ...
                if (faction != null && TOCFacTags.Contains(faction.Tag))
                {
                    ...
                    foreach (var store in stores)
                    {
                        ...
                        {
                            if (BannedItems.Contains($"{offer?.Item?.TypeIdString}{offer?.Item?.SubtypeId}"))
                            {
                                Core.Log.Info($"Clearing {offer.Item.Value.SubtypeId}");
       ...
       throw new Exception("Failed to find patch method InsertStoreItem");
        ...
        internal static readonly MethodInfo insertPatch =
            typeof(TradeOperatorsStationFakerStartupClean).GetMethod(nameof(StorePatchMethod), BindingFlags.Static | BindingFlags.Public) ??
        ...
        public static Boolean StorePatchMethod(MyStoreBlock __instance, IMyStoreItem item)
        {
            ...
            if (TOCFacTags.Contains(owningFac.Tag))
            {
                ...
                if (BannedItems.Contains($"{storeItem?.Item?.TypeIdString}{storeItem?.Item?.SubtypeId}"))
                {
