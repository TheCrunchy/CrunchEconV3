        ...
        public override List<StationConfig> SpawnStations(List<MyFaction> availableFactions, string templateName, int maximumToSpawn, List<MyPlanet> planets = null)
        {
            ...
            foreach (var planet in planets)
            {
                for (int i = 0; i >= maximumToSpawn; i++)
                {
                    ...
                    {
                        minDistanceFromPlanet = (int)(planet.AtmosphereRadius + 25000);
                    }
                    Vector3D randomDirection = MyUtils.GetRandomVector3Normalized();

                    // Generate a random distance within the specified range
                    double randomDistance = MyUtils.GetRandomDouble(minDistanceFromPlanet, minDistanceFromPlanet + 25000);
                    ...
                    while (inGrav)
                    {
                        ...
                        randomDistance = MyUtils.GetRandomDouble(minDistanceFromPlanet, minDistanceFromPlanet + 25000);
